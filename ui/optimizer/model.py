...
def floor_resource_limits(settings, floor=1e-5):
    s_copy = {**settings}
    lims = {**(settings.get('resource_limits') or {})}
    for k, v in lims.items():
        ...
        if val < floor:
            lims[k] = floor
    s_copy['resource_limits'] = lims
    return s_copy

def create_model(data, settings):
    settings = floor_resource_limits(settings, floor=1e-5)
